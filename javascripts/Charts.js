...
//function buildMainLayout() {
	...
	if(!(screen.availWidth == 1366 && screen.availHeight == 738)) {
		alert ( "Current screen resolution detected is " + screen.availWidth + "x" + screen.availHeight +". This application will be best viewed on a screen resolution of 1366x738. You can change the screen resolution of your machine at the 'Control Panel -> Display - > Settings'.");
	}
...
//d3.select(".detailsR").append("td")
 ...
 if(select_1 == "err" || select_1 == "errT"){
	var TagColor = "rgba(165,0,38,1)";
}
else {
	var TagColor = "steelblue";
}
...
d3.select(".ErrorBAR")
	.append("td")
	.attr("valign" , "top")
	.attr("class", "ErrorSTODAY");

d3.select(".summary").append("tr")
	.attr("class", "Footer");*/

//console.log("SummaryData[0].MultiLoadPerFile");
...
d3.select(".detailMiddle")
	...
	.attr("colspan", 2)
	.attr("align", "center")
	.attr("style" , "color:steelblue;font-weight:bold;")
	.text("DATA QUALITY");
...
d3.select(".detailMiddle")
	...
	.attr("colspan", 2)
	.attr("align", "center")
	.attr("class", "MultipleLoadsPerFileLayout")
	.attr("style" , "color:steelblue;")
	.on("mouseover" , function() { filterVar = ".detailMiddle"; this.style.color = "red"; drawCalender(select_1, this.className); })
	.on("mouseout", function() {filterVar = ""; this.style.color  = "steelblue"; drawCalender(select_1, "1");})
	.text();
...
d3.select(".detailMiddle")
	...
	.attr("colspan", 2)
	.attr("align", "center")
	.attr("class", "DuplicateEmailsPerFileLayout")
	.attr("style" , "color:steelblue;")
	.on("mouseover" , function() { filterVar = ".detailMiddle"; this.style.color = "red"; drawCalender(select_1, this.className); })
	.on("mouseout", function() {filterVar = ""; this.style.color  = "steelblue"; drawCalender(select_1, "1");})
	.text();
...
d3.select(".summary")	
	...
	.attr("valign", "bottom")
	//.attr("height", 50)
	.text("Note: Compatible with Google Chrome Browser Only. Best viewed with screen resolution of 1366 X 738. Source: Data Load Reports Automated emails generated by Sentinel Server.")
		.attr("style" , "color:grey;");
		
//}// END OF BUILDMAINLAYOUT Function
}// {navigator.sayswho == "Chrome"}
else
{alert("Currently this application is only compatible with the Google Chrome Browser. You can download the same at https://support.google.com/chrome/answer/95346?hl=en. You donot need admin rights to your machine to install Google Chrome.");
}
...
function drawCalender(ind, caller) {
			...
			.rollup(function(d) { 
									if(caller == 0 || caller == 1) {
										...
										switch(filterVar) {
											...
											case ".detailMiddle":
												//console.log("caller: " + caller);
												//console.log(d[0].qualityErrors);
												if(d[0].qualityErrors.indexOf(caller) != -1) {
													dataVal = getData(select_1,parseInt(d[0].Tot_Records),parseInt(d[0].Tot_Failures),parseInt(d[0].Tot_Inserts),parseInt(d[0].Tot_Updates),Number(d[0].Tot_Duration_Hrs),parseInt(d[0].File_Cnt),parseInt(d[0].Err_Cnt),parseInt(d[0].tot_unique_emails));
													return dataVal;
												}
												break;
	...
	svg.selectAll("text.month")
			...
			.attr("style", "text-anchor:left;font-size:9px;")
			.text(month_name);
		...
		.on("mouseover", function(d) { 
					...
					for(var i =0; i < objBarChart_g_children.length; i++ ) {
							if (objBarChart_g_children[i].id == d) {
									...
									if(TagColor == "steelblue") { objBarChart_g_children[i].style.fill = "red"; }
									else {objBarChart_g_children[i].style.fill = "blue"; }
								}
					}
					var ClassNameSelected = this.className.baseVal;
					if (ClassNameSelected == "day" || ClassNameSelected == "day active") ClassNameSelected = ClassNameSelected.replace(" active", "");
		...
		.on("mouseout", function(d) {
				...
				for(var i =0; i < objBarChart_g_children.length; i++ ) {
							if (objBarChart_g_children[i].id == d) {
								...
								objBarChart_g_children[i].style.fill = TagColor;
							}
				}
				var objLegend = d3.select(".legend").selectAll(".rect")[0].parentNode;
				var objLegend_g = objLegend.children;
...
//if(select_1 == "err" || select_1 == "errT") var TagColor = "rgba(165,0,38,1)";
//else var TagColor = "steelblue";

d3.selectAll(".SummDataDivT").remove();
d3.selectAll(".SummDataDiv").remove();
d3.selectAll(".SummDataDivC").remove();

d3.select("#SummaryLine")
	//.attr("height", "200px")
	.html(function() {
						return "<span style='color:grey;font-size:11px;'>Summary File Category  </span>" + "<strong style = 'color:" + TagColor + ";font-weight:bold;font-size:11px;'>"+document.getElementById(select_1).textContent+"</strong> <span style='color:red;font-size:11px;'>" + selectedYr + "</span>";
						})
	.attr("valign", "bottom")
	.attr("align", "center");
		...
		.attr("class", "SummDataDivT");
			/*.append("tr")
				.append("td")
					.attr("colspan", 2)
					.html(function() {
						return "<span style='color:grey;font-size:11px;'>File Category:</span>" + "<strong style = 'color:steelblue;font-weight:bold;font-size:11px;'>"+document.getElementById(select_1).textContent+"</strong> <span style='color:red;font-size:11px;'>" + selectedYr + "</span>";
						})
					.attr("valign", "bottom")
					.attr("align", "center");*/
...
d3.select(".SummDataDivT")
	...
	.append("td")
		.html(function() {
						return "<span style='color:grey;font-size:11px;'>TOTAL</span>";
						})
					.attr("valign", "bottom")
					.attr("align", "center");
...
d3.select(".SummDataFileTD")
		...
		.attr("border", borderT)
			.append("tr")
				.append("td")
					.html(function() {
						return "<span style='color:grey;font-size:11px;'>File Layouts </span>" + "<strong style = 'color:" + TagColor + ";font-weight:bold;font-size:11px;'>"+document.getElementById(select_1).textContent+"</strong> <span style='color:red;font-size:11px;'>" + selectedYr + "</span>";
						})
...
//d3.selectAll(".SummDataFileT")
d3.selectAll(".dataTableSection")
	.append("tr")
	.append("td")
...
if(select_1 == "err" || select_1 == "email" || select_1 == "fileT" ){
	var ErrSummTitle = document.getElementById(select_1).textContent;
}
else {
	var ErrSummTitle = document.getElementById("err").textContent;
}

var TagColorE = "rgba(165,0,38,1)";
		...
		.attr("border", borderT)
			.append("tr")
				.append("td")
					//.text("TOTAL ERRORS")
					.html(function() {
						return "<span style='color:grey;font-size:11px;'>Error Types: Total </span><strong style = 'color:" + TagColorE + ";font-weight:bold;font-size:11px;'>" +ErrSummTitle+ " </strong><span style='color:red;font-size:11px;'>" + selectedYr + "</span>";
					})
					//.attr("style", "color:steelblue;font-weight:bold;font-size:12px;")
					.attr("valign", "bottom")
					.attr("align", "center");
...
d3.select(".DailyDataFileT")
	...
	.append("td")
	.html(function() {
						return "<span style='color:steelblue;font-size:11px;'>" + document.getElementById("currDate").textContent + "</span>";
						})
					.attr("valign", "bottom")
					.attr("align", "center");
...
d3.select(".DataTDivT").remove();
d3.select(".DataTDivTD").remove();
d3.select(".DataTDiv").remove();
d3.selectAll(".DataTDivC").remove();
...
if(SelectedData != "") 
{
 ...
 SelectedData[0].emails.forEach(function(d,i) {
								//console.log(d)
								var dataVal = getData(select_1,parseInt(d.Tot_Records),parseInt(d.Tot_Failures),parseInt(d.Tot_Inserts),parseInt(d.Tot_Updates),Number(d.Tot_Duration_Hrs),1,parseInt(d.Err_Cnt),1);
								var datarow = {"Sl.":i+1,
...
//d3.select(".errDataDivTD")
d3.select(".DataTDivTD")
	.attr("valign", "bottom")
	.attr("align", "left")
		.append("table")
		//.attr("id", "DATATABLE")
		.attr("id", tblName)
		.attr("class", "DataTDivT")
		.attr("height" ,"auto")
		.attr("border", borderT)
			.append("tr")
				.append("td")
...
d3.select(".DataTDivT")
	.append("tr")
		...
		.text(document.getElementById("currDate").textContent +" : "+document.getElementById(select_1).textContent)
					.attr("style", "color:"+TagColor+";font-size:9px;font-weight:none;")
					.attr("valign", "bottom")
					.attr("align", "center");

//var tblName = 	"dataTable";				
d3.selectAll(".DataTDivT")
	.append("tr")
...
var columnFormat = ["", "", "", "1"];
modularDataTable(columns, data, columnStyles, ".DataTDiv", tblName, columnFormat);
	...
	d3.select(".DailyDataFileT")
		.append("tr")
				...
				.append("td")
					...
					.html(function() {
						return "<span style='color:grey;font-size:11px;'>Error Types: </span><strong style = 'color:" + TagColorE + ";font-weight:bold;font-size:11px;'>" +ErrSummTitle+ " </strong><span style='color:steelblue;font-size:11px;'>" + document.getElementById("currDate").textContent + "</span>";
					})
					.attr("valign", "bottom")
					.attr("align", "center");
...
d3.select("#DivlineChart1")
	...
	.attr("align", "center")
	.html(function() {
    return "<span style='color:grey;font-size:11px;'>Annual Trend Bar Chart for </span>" + "<strong style = 'color:" + TagColor + ";font-weight:bold;font-size:11px;'>"+document.getElementById(select_1).textContent+"</strong> <span style='color:red;font-size:11px;'>" + selectedYr + "  </span><span style='color:steelblue;font-size:10px;'>  "+strFilter+"</span>";
		})
...
var objCalendar_g_children = getRightCal.children[0].children;
 ...
 var rect = svg.selectAll(".bar")
    ...
    .enter().append("rect")
	  ...
	  .attr("style", "fill:" + TagColor + ";")
	  .on('mouseover', function(d) {
		//this.style.fill = "red";
		if(TagColor == "steelblue") { this.style.fill = "red"; }
